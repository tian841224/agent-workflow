...
本流程只套用於涉及專案或程式工作的任務，例如讀寫 repo／工作區、修改程式碼或設定、執行 build／test、除錯、技術審查，以及專案規劃或規格整理。

不涉及專案或程式工作的任務（例如一般知識問答、翻譯、文字潤稿、生活規劃或一般腦暴）由主對話直接處理：不判定 L0／輕軌／標準軌／重軌、不宣告或切換角色、不建立 acceptance 文件，也不執行 reviewer／QA／PM／debugger 流程。

若同一請求混合兩類工作，只對其中涉及專案或程式的部分套用本流程，其餘部分由主對話直接處理。只有通過本 gate 後，才進入下方的流程分級與角色分派。
