...
Node.js 20 以上版本是唯一 runtime family（對應 `package.json` 的 `engines.node: ">=20"`）。`src/` 是 TypeScript source，`dist/agent-workflow.mjs` 是唯一可部署 ESM bundle；已安裝的 hooks 只執行記錄在 managed state 的 Node 絕對路徑與該 bundle，不依賴 repo、`node_modules` 或 npx cache。
