...
`.agents/` 文件品質由 AGENTS pointer、`writing-for-agents`、review 與 contract lint 維持，不再保存或驗證 session read proof，也沒有 PostToolUse／SessionEnd guard。三平台共用同一組 hook 功能矩陣：工具呼叫的 guard、回覆結束時的 localization lint，以及依任務自動篩選的 memory hook。Claude 的 guard matcher 只列 shell、編輯類與 `mcp__.*` 工具（唯讀工具本來就一律放行），Codex／Antigravity 的工具名稱未固定，維持 `*`。Claude Stop payload 不含回覆文字，lint 由 payload 的 transcript 路徑檔尾取最後一則 assistant 文字，並以頂層 `decision: "block"` 回應。用語對照表由 `locale-context` 在 Claude／Codex 的 `SessionStart` 與 Antigravity 首次 `PreInvocation` 注入，來源與 lint 相同（安裝時產生的 policy），取代原本「首次中文回覆前讀 skill 與 locale.md」的步驟；Claude／Codex 在 `SessionStart`（只注入用語對照表，不掃描記憶）、`UserPromptSubmit`、`PreToolUse`、`Stop` 觸發；Antigravity 沒有等價的 prompt-submit lifecycle，才在首次 `PreInvocation` 提供主題導航，其餘以 `PreToolUse`／`PostInvocation` 做必要映射。
