...
`OrchestrationEngine` 以 protocol 3 提供 deterministic multi-worker control plane，並保留 protocol 2 的 experimental phase-tracker 相容性。protocol 3 由 coordinator 驗證至少兩個獨立 worker、檔案 ownership 不重疊、沒有順序相依與 shared persistent state，接著用暫時 `GIT_INDEX_FILE` 建立 dirty-worktree snapshot、建立 detached worktree、child task 與 ExecutionPacket，經 `Bind`／`worker-check`／`worker-exec`／`Collect`／`Integrate`／`Apply`／`Cleanup` 管理 worker-level state、immutable artifacts、衝突與 parent fingerprint。其 state 與 task state 走同一把檔案鎖，狀態 authority 是 `schemas/orchestration.schema.json`；parent 的 task-gate、reviewer、最終驗證與 close-task 仍由 coordinator 在 Apply 後統一執行。三平台 native dispatch 只負責啟動並回傳 stable `run_id`，不重新解釋 ExecutionPacket 或 workflow policy。
