...
任務開始時由主對話（orchestrator）直接依本節判準判定軌別並向使用者宣告，不為判軌 spawn 任何 agent，使用者可否決；使用者也可以在提出需求時直接指定軌別，跳過判定（orchestrator 仍需檢查指定軌別是否明顯不符判準，不符時要提出疑慮，但不強迫改判）。architect 被 spawn 後若發現軌別與實際規模明顯不符，停手回報 orchestrator，不自行改判。判定為複雜的架構決策（重軌）時，orchestrator 於宣告軌別時一併提醒使用者可改用高階模型呼叫 architect 取得更完整的方案比較。判斷不出一律往上升一級（L0 有疑慮升輕軌、輕軌與標準軌之間判斷不出升標準軌、標準軌與重軌之間判斷不出升重軌）。判軌前，若專案記憶索引（`~/.claude/projects/<project-slug>/memory/MEMORY.md`）含 `overview.md`，須先讀取以掌握專案脈絡與高風險區（此檔亦是判斷「高風險關鍵寫入路徑」的資訊來源之一，見 §9）。

**判軌核心原則**：判軌不以檔案數、模組數、行數為依據，看兩件事——「這次改動是不是**單一功能**」與「出錯時會**波及多少既有功能**（blast radius）」。乾淨架構等分層框架下，一個功能垂直跨多層／多模組仍屬單一功能，不因此升軌。

- **功能的操作型定義**：使用者可觀察的一項行為／一個 use case／一個對外入口點（指令、endpoint、畫面操作），不是一個模組或一個檔案。
- **影響的定義＝出錯時波及範圍**：不是「意圖改變哪些功能」，而是「這次改動若出錯，會壞到哪些功能」——行為不變的重構動到多功能共用路徑，一樣算波及多功能。
- **判軌依據分級**：L0 免附；輕軌宣告時一句話說明影響侷限的理由；標準軌以上必附「改動所屬功能＋受影響既有功能清單」，動到共用元件時以 grep 呼叫點確認其他使用者，不得憑印象。DI／interface 分派／event bus 等靜態查不出完整呼叫點時，視為「確認不了影響範圍」→ 升軌，不得當作沒有影響。

**L0 微軌**（全部符合才適用）：
- 改動不影響任何功能的可觀察行為
- 不新增/修改邏輯分支、不改介面/契約、不改 DB schema、不碰高風險關鍵寫入路徑
- 類型限定：文案/註解/log 訊息、設定值調整、顯而易見的 typo 或一行修正、純樣式微調
- 軟護欄：規模大到需要逐條核對才能確認沒改到行為時，升輕軌
...
- bug fix（行為修正）或既有**單一功能**內的小幅改動（不新增對外行為），出錯波及範圍侷限該功能自身
- 共用元件 bug fix 例外：把錯誤行為修回規格預期，即使多個功能因此「變正確」，仍可走輕軌；但 L2 微驗收清單必須涵蓋每個受影響功能至少一條驗證，受影響功能約 >5 個或碰高風險路徑仍升軌
- 不修改既有對外 API/WS 契約、不改 DB schema
- 不涉及高風險關鍵寫入路徑

**標準軌**（全部符合；規模超過輕軌但不到重軌門檻）：
- **單一功能**的新增或行為變更；實作可垂直跨多個模組/分層（乾淨架構切片），不因此升重軌
- 出錯波及範圍侷限本功能——若動到共用元件，須以呼叫點證據確認其他使用功能不受波及，確認不了就升重軌
- 不修改既有對外 API/WS 契約、不改 DB schema（新增只服務本功能的全新 endpoint 可；往既有 WS channel 新增訊息型別，若既有消費者對未知型別不容錯，視同修改既有契約）
- 需求本身明確，不需要 PM 多輪釐清即可動工
- 不涉及高風險關鍵寫入路徑

**重軌**（任一命中）：
- **波及多個功能**：改動共用元件/核心抽象/橫切關注點，出錯時會壞到多個功能（含行為不變的大範圍重構），或一次交付多個功能（彼此獨立的多個小功能應優先建議拆成多個任務各自判軌，而非合併成一個重軌）
- **修改既有對外 API/WS 契約**（既有消費者會受影響；純新增且無既有消費者受影響不算）、改 DB schema（任何變動皆算）
- 觸及高風險關鍵寫入路徑（不可逆或高影響的資料異動，如帳務、交易、權限；具體範圍由各專案 CLAUDE.md 定義）

**附加 gate：畫面驗證**（不是獨立軌別，掛在其他軌別後面）——任一軌別的任務只要**含前端功能修改**（非純樣式微調，那屬於 L0），流程尾端一律追加畫面驗證：
- L0/輕軌：qa（或主對話直接）用 browser 工具依變更說明走一次使用者流程核對即可，PM 不出場、不需凍結任何文件
- 標準軌/重軌：qa 先執行 mini-spec.md／checklist.md 的 `type: ui` 條目、PM 再依此複核
- 純樣式微調（顏色、間距等不影響行為的調整）不觸發此 gate，仍走 L0

**中途升降軌**：實作途中發現命中更高軌判準（例如輕軌做到一半才發現要修改既有對外契約），立即停手、向使用者宣告升軌理由，補走該軌缺的前置步驟（標準軌補 mini-spec、重軌補完整 spec.md 流程並凍結）後再繼續，不得邊做邊補、不得沿用低軌別的把關标準完成高軌別的改動。降軌（例如重軌判定後發現規模其實只需標準軌）需先取得使用者同意才能降級，不得自行決定。

## 2. 輕軌（L1–L4）

```
L1 判定：orchestrator 依 §1 判準判定軌別並向使用者宣告（不 spawn agent）
...
PM、QA 不出場（除非觸發上方「附加 gate：畫面驗證」——此時僅 qa 出場做畫面核對，PM 仍不出場）；不建 acceptance 目錄。

## 3. 標準軌（M1–M4）

填補輕軌與重軌之間的空隙：規模超過輕軌小幅修正，但仍是單一功能、出錯波及範圍侷限本功能、不修改既有契約/schema、需求已明確，不需要重軌整套 SDD 雙凍結與四次以上使用者往返。標準軌不比照重軌拆「PM 寫意圖、architect 補手段」兩段——這是刻意精簡：需求本身已明確不需要 PM 介入釐清，由 architect 一人寫完 mini-spec 即可，單人設計驗收條目的盲點交給 qa 的探索性測試補償（見 M4）。

```
M1 mini-spec：architect 直接寫一頁規格（`templates/mini-spec.md` 模板）——目標、
...
R5 驗收：
   ...
   - qa 清單跑完後加探索性測試，**以主動找出 bug 為目標**，以一輪為預算——自行推理
     「這個功能最可能在哪裡壞掉」挑 3–5 個最可疑情境針對性測試；該輪有發現（FAIL 或
     規格缺漏）可再加一輪，連續一輪無發現即收尾，不無限探索：
     前端做畫面探索（非清單操作順序、快速連續操作、中途切換頁面再回來、重新整理後
     狀態是否正確）；後端做 edge-case 探索（清單外的異常輸入、邊界值、特殊字元、
     超大/負值、並發或重複請求、前後狀態依賴）
