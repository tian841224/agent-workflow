...
| [`clean-comments`](.agents/skills/clean-comments/SKILL.md) | 必裝 | 核心規則常駐於 `AGENTS.md`；Claude 額外由 PreToolUse prompt hook（Edit\|Write）在寫入前強制 SKILL.md 的 enforcement block，違規時擋下該次寫入並要求就地修正，不必等到寫完才被發現；只有多行註解、public/doc comments、高風險判斷依據或 comment pollution 才需載入完整 skill 查邊界案例。 |
