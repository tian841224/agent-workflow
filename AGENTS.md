...
- 所有中文自然語言輸出一律使用臺灣慣用繁體中文，不使用簡體字或中國特有用語；技術詞彙沒有自然臺灣譯名時保留英文，保留程式碼與技術術語；中文敘述用全形標點，語氣自然直接；圖示能明顯提高理解時才用 ASCII。這條核心規則常駐，每次提交訊息時，hook 會注入 localization-tw skill 的核心規則與用詞清單（例如「落地」不可用），輸出中文前先讀完再寫。<!-- skill:localization-tw -->
