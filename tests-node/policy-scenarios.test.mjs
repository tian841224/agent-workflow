...
for (const file of fixtures) {
  ...
  test(`policy scenario: ${fixture.scenario}`, () => {
    const compiled = compile(fixture.task);
...
// the Reviewer included, or a docs/read-only bypass would still owe evidence.
...
// A confident, local, risk-free code change owes only the Reviewer: its runtime receipts come from the
// task's own acceptance cases, which the gate checks outside the plan. Larger baseline and regression
// capabilities remain classification-driven.
test("a confident, local, risk-free code change requires only the Reviewer", () => {
  for (const name of ["normal-bugfix", "local-feature"]) {
    ...
    assert.deepEqual(fixture.expected.required, ["reviewer"], name);
    assert.deepEqual(fixture.expected.required_evidence, ["role.reviewer"], name);
  }
});
...
test("a change the agent is unsure about pulls in impact discovery and baseline validation", () => {
  ...
  assert.deepEqual([...fixture.expected.required].sort(), ["baseline_validation", "impact_discovery", "reviewer"].sort());
});
...
// The durable contract a scenario can pin is that the financial flag selects MV4 and that MV4 still
// declares runtime_execution, which is what makes it a gate proof recorded by `evidence-run` rather
