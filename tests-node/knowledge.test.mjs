...
test("memory-context for one project never includes a knowledge entry written under a different project id", () => {
  ...
  const upsert = (projectId, topic, content) => run(["knowledge", "--action", "Upsert", "--state-root", state, "--project-id", projectId, "--topic", topic, "--content", content, "--status", "verified"]);
  assert.equal(upsert(currentProjectId, "current-project-fact", "This project's own memory entry.").status, 0);
  assert.equal(upsert(otherProjectId, "other-project-secret", "OTHER_PROJECT_DISTINCTIVE_TEXT").status, 0);
...
test("a freshly-created knowledge entry defaults to status candidate and is excluded from memory-context until verified", () => {
  ...
  const verified = upsertGlobal(["--topic", "verified-fact", "--content", "VERIFIED_DISTINCTIVE_TEXT", "--status", "verified"]);
