...
test("runtime operation authorization: install/repair/skill are exempt from skill-guard, evidence-run is not", () => {
  const root = join(tmpdir(), `agent-workflow-runtime-auth-agents-${process.pid}-${Date.now()}`);
  mkdirSync(root, { recursive: true });
  const cliBinary = readFileSync(cli);
  writeFileSync(join(root, "managed-runtime.json"), JSON.stringify({ runtime_hash: createHash("sha256").update(cliBinary).digest("hex") }));
  // install/repair are the sanctioned .agents writers: verified identity + this subcommand exempts it.
  assert.equal(denied("skill-guard", `node ${cli} repair --root .agents`, ["--state-root", root]), false);
  // evidence-run has no legitimate business writing .agents; a verified identity must not exempt it
  // just because its trailing command happens to mention a .agents path.
  assert.equal(denied("skill-guard", `node ${cli} evidence-run --task-path t/task.json --requirement-id x --summary y -- rm -rf .agents/skills/x`, ["--state-root", root]), true);
});
