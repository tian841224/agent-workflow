...
| lite（使用者指定的重軌精簡替代） | [lite.md](lite.md)（含 LT1–LT4、三項必答自審） |
...
- **lite**（不自動判入，僅使用者主動指定，如「走 lite」）：重軌等級任務的單對話精簡模式——單檔 mini-spec 凍結、主對話直做規格與實作、審查與驗收平行派唯讀 agent；品質規則同重軌，省的是文件數、角色 spawn 與確認往返。
