...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agent-workflow-contract-") as temp:
        ...
        decision = json.dumps({"final_action": declared_plan["final_action"], "selected": [{"name": item["name"], "kind": item["kind"], "steps": [step["id"] for step in item["steps"]]} for item in declared_plan["selected"]], "suppressed": [item["name"] for item in declared_plan["suppressed"]], "unknown": [item["name"] for item in declared_plan["unknown"]], "complexity": declared_plan["complexity"]}, separators=(",", ":"))
