...
export function orchestrate(options: Map<string, string | boolean | string[]>): number {
  ...
  const phase = String(state.phase) as Phase; const target = aliases[action] || action.toLowerCase() as Phase; if (!(transitions[phase] || []).includes(target)) throw new Error(`invalid OrchestrationEngine transition: ${phase} -> ${target}`);
