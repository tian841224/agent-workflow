...
export function taskPath(value: string): string {
  if (value.endsWith(".json")) return resolve(value);
  // Agents often pass the task.md they just wrote; it names the same task directory.
  return join(basename(value) === "task.md" ? dirname(resolve(value)) : resolve(value), "task.json");
}
