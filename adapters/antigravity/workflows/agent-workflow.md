...
4. For an Elevated task, before reading the code run `~/.agent-workflow/runtime/scripts/project-doc.ps1 -Action Lookup -Paths '<paths>'` and record the matched docs under `## Project docs` - `read:`. Standard tasks use current code, callers, and tests without a mandatory project-doc lookup.
...
6. Run relevant tests and, for code tasks, the managed `pre-review.ps1`; do not continue to review or completion after a failure.
7. Run Reviewer then Verifier for every code task. Add Adversarial, browser, mutation, full execution-path review, or fingerprint only when the task is Elevated or its risk flags require it.
...
9. Run Retrospective only for a suspected regression, repeated fix, or explicit request. Record its result and use `retro.ps1 -Action Record` only for a confirmed regression.
10. Standard tasks may be marked `done` after completion criteria, validation, Reviewer, and Verifier are complete. Coordinator/worker or Elevated tasks use `close-task.ps1` for the full legacy gate. Use `paused`, `blocked`, or `superseded` truthfully.
