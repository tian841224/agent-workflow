...
7. The main conversation selects the necessary evidence capabilities and any subset of Reviewer, Adversarial, and Verifier for each code task. Record the complete selection in `workflow_mode: main` and `workflow_request`, then run only those selected capabilities in their declared order.
