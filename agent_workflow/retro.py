...
def set_frontmatter_field(text, name, value):
    replacement = f"{name}: {value}"
    pattern = rf"(?m)^{re.escape(name)}:.*$"
    if re.search(pattern, text):
        return re.sub(pattern, replacement, text, count=1)
    return re.sub(r"(?m)^---\s*$", replacement + "\n---", text, count=1)
def main(argv=None):
    ...
    if a.action=="Resolve":
        ...
        text=finding.read_text(encoding="utf-8-sig"); text=set_frontmatter_field(text,"status",a.status); text=set_frontmatter_field(text,"resolved_at",time.strftime("%Y-%m-%dT%H:%M:%S%z")); text=set_frontmatter_field(text,"resolution_note",json.dumps(a.note,ensure_ascii=False)); finding.write_text(text,encoding="utf-8",newline="\n"); entry["status"]=a.status; save(index_path,data); print(json.dumps({"ok":True,"id":a.id,"status":a.status},ensure_ascii=False)); return 0
