...
task 建立後先用一次 `agent-workflow preflight --task-path <path> --repo-root <repo-root>` 排除固定環境問題。實作期間只保留必要的局部驗證回饋；所有 slices、檔案、測試與文件都穩定後，才執行最終回歸，並依 [review.md](review.md) 的 task-level timing 執行 Reviewer 與 delivery receipt。receipt 產生後若交付內容再變動，依 freshness 規則重新執行所需的 runtime checks。
