...
Runtime 依 `impact_scope`／`impact_effect`／`impact_confidence`／`risk_flags` 透過 `workflow-policy.json` 的 `require_when` 算出一組 `required` capability，這是不可省略的下限。`managed_change: true` 本身不強制任何 capability：單檔、局部行為、高信心且無 risk flag 的修改 `required` 為空，`impact_confidence` 降為 `medium`／`low`、影響面擴大或命中高風險 flag 才會讓 runtime 開始強制；主對話依已知需求與程式脈絡，在 `workflow_request` 裡疊加想額外執行的 capability，只能加、不能拿掉 `required` 命中的項目。單純不把某個 capability 寫進 `workflow_request` 並不會讓它從 gate 消失——`required` 由分類欄位直接算出，要移除只能明確執行 `waive`（需 `--requirement-id <被豁免的 requirement>` 與 `--confirmed-by-user`），而這個 waiver 同時綁定當下的 `plan_hash` 與 `plan_revision`，task 的分類一變（risk flags、`impact_scope`、`impact_effect` 等任何進到 hash 的欄位改變）就自動失效，需重新確認。`plan_revision` 是必要的第二個條件：`plan_hash` 是分類的純函式，分類改走一圈再改回來會還原同一個 hash，只比對 hash 會讓舊 waiver 復活。

**外層選取（跑哪些 capability）**：主對話依 task metadata、`workflow_facts` 與實際程式脈絡判斷 `suggested` 候選並確認、覆寫或補充，寫入 `workflow_request`；runtime 再把 `required` 與 `workflow_request` 合併成最終 `selected`。`required` 為空時任何組合都合法——只跑 evidence capability 而沒有 `reviewer`、或一個都不跑（此時 `## Impact surface` 必填，說明為何判斷這個 task 不需要任何 capability），都是正常結果；`required` 非空時，`selected` 至少涵蓋這些項目，`workflow_request` 只能疊加、不能覆寫。
...
|---|---|---|
| 單檔 bug fix、模組內小功能 | 無 required；是否加開 `reviewer` 由主對話決定 | 0 |
| 判斷為 isolated 的小改動（例如無 consumer 的 additive 欄位） | 無 capability ＋ `Impact surface` 說明判斷依據 | 0 |
| 邏輯單純但需實測、無呼叫端 | 無 required；主對話自行決定要跑的驗證 | 0 |
| 分類不確定（`impact_confidence` 為 `medium`／`low`） | `impact_discovery` → `baseline_validation` | 6 |
| 純測試變更命中 test integrity risk | `test_integrity` → `baseline_validation` | 6 |
...
Reviewer 用於判斷完整 diff 是否符合需求、影響面與失敗模式，並從 real entrypoint 確認完成條件與可觀察結果。命中 `financial`、`data_write`、`migration`、`irreversible`、`schema`、`contract` 時，在同一位 reviewer 的指令裡明寫要推翻的資料溯源、底層語意或同型擴散假設——runtime contract 只認得 `role.reviewer` 一種身份，不存在第二位 reviewer。每列都是最終交付批次的典型組合，不是逐一實作階段的 pipeline。
