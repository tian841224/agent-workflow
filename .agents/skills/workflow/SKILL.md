...
3. 一般 logic change 只遵守 `AGENTS.md` 的短註解規則；只有新增／重寫多行註解、public/doc comments、高風險判斷依據，或 review 發現 comment pollution 時才讀 [clean-comments](../clean-comments/SKILL.md)。只有 execution packet 指向 [project-docs](../project-docs/SKILL.md) 時才做 Lookup → 讀取 → Remember；高信心、file-local、local_behavior 且無相關高風險邊界的 code change 不為了 `code_change: true` 單獨付 project-doc 成本。
