...
本 skill 只有在實際修改「目標專案」的 application source code logic 或 test code logic 時啟用（例如 Golang、Vue、TypeScript 等程式語言撰寫的產品邏輯）。此類任務必須建立 `task.md`，並依序執行 Reviewer → Verifier；命中既有 risk flag 時，依規則加入額外角色。

除上述情況外，其餘任務一律 bypass workflow（包含但不限於：指令執行、script 修改與操作、註解、文件、設定、測試調查或執行、除錯分析、code review、規劃、問答與翻譯）。這些任務不建立 task、不執行 Reviewer、Adversarial、Verifier 或 Retrospective，由主對話直接處理。判定為 bypass 後，停止本 skill，不套用後續 task 流程。
...
0. 本 skill 僅適用於實際修改「目標專案」application source code 邏輯或 test code 邏輯的任務；除此之外的 non-code tasks（含純註解修改、設定／文件修改、script 修改與操作、測試調查、除錯分析、code review、規劃、問答與翻譯）一律 bypass workflow，由單一主對話直接處理，不建立 task 或啟動角色。
...
- Elevated task 讀相關程式前才跑 `~/.agent-workflow/runtime/scripts/project-doc.ps1 -Action Lookup -Paths '<預計改動的 repo 相對路徑>'`，先讀命中的模組文件再讀 code；`stale: true` 只當線索，一律以現況程式為準。Standard task 只需讀專案 instructions、相關程式、呼叫端與既有測試；若發現架構或影響面不明，再升級為 Elevated task。
...
`code_change: true` 時，依序啟動原生 `agent-workflow-reviewer` 與 `agent-workflow-verifier`。`risk_flags` 命中第 3 節列的六個旗標任一時，Reviewer PASS 後才加開原生 `agent-workflow-adversarial`（見 6a）。三者皆唯讀，輸入只帶 task、diff、必要專案規則與驗證證據。`code_change: false` 跳過全部角色。各角色的審查方法論（Reviewer 的八面向與 execution path 重建方式、Adversarial 的四項檢查、Verifier 的探索範圍）定義在各自的角色檔案，本節不重複，只記錄何時觸發與主 agent 收到結果後的動作。`code_change: true` 的每次 bug fix 或邏輯調整都必須依第 4 節 TDD 規則新增或修正對應測試——該測試在修復前必須真的失敗（證明正確捕捉到這次的錯誤），修復後變綠（證明能擋住同樣錯誤再次發生）；Reviewer 的 Code quality and conventions 面向會核對這件事有沒有做到。
