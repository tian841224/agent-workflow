...
3. 一般 logic change 在實作期間只遵守 `AGENTS.md` 的短註解規則；只有新增／重寫多行註解、public/doc comments、高風險判斷依據，或 review 發現 comment pollution 時才主動讀 [clean-comments](../clean-comments/SKILL.md) 查邊界案例。Claude 在每個任務準備結束時另由 Stop agent gate 讀取完整 clean-comments skill，僅檢查本次 diff 新增或修改的註解作最終合規驗證。只有 execution packet 指向 [project-docs](../project-docs/SKILL.md) 時才做 Lookup → 讀取 → Remember；高信心、file-local、local_behavior 且無相關高風險邊界的 code change 不為了 `code_change: true` 單獨付 project-doc 成本。
