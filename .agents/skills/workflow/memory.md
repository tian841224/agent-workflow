...
1. Claude／Codex 只有 UserPromptSubmit：用 payload 的 prompt 與 cwd 依當前任務自動篩選 curated verified 記憶（最多 6 筆、800 字元）。自然語句用英文／中文斷詞後匹配內容；不將 prompt 插入 shell。每次提交都重新篩選，沒有命中就不注入。Antigravity 沒有等價的 prompt-submit 事件，才在首次 PreInvocation 提供主題導航作為替代入口。
