...
**Experimental.** `src/orchestration.ts` today only tracks phase state (`planned -> split -> executing -> integrating -> integrated -> cleaned`); it does not yet implement the dirty-worktree snapshot, `GIT_INDEX_FILE`, or patch collect/apply steps described below. `orchestrate --action Init` (runtime-created worktrees) refuses to run unless `AGENT_WORKFLOW_ORCHESTRATION_EXPERIMENTAL=1` is set; `RegisterNative` stays available because the host, not the runtime, creates those worktrees; until this document's flow is actually implemented, dispatch workers manually instead (host sub-agent tool bound to per-worker worktrees, collected by hand).
