...
- 在 `## Scope` 下補 `### Non-goals and compatibility`；`### Acceptance cases` 本來就是每個 code task 都要寫的。這兩段位於 `intent_hash` 涵蓋的區域，使用者批准的非目標與驗收案例才不會在批准後被改掉而雜湊不變；影響面、方案與證據留在 task.json 的分類、plan 與 evidence。
...
- 通過雜湊比對前，目標、非目標與完成條件維持不動；需求變更時 supersede 舊 task 並建立新 task。
...
釐清流程見 workflow skill 的第 2 步。使用者在問答中確認目標與驗收標準之後，先寫 task.md 再執行 `task-init`；接著執行 `approve-intent --as-user --confirmed-by "user message"`，把 attestation 升級為使用者確認。
