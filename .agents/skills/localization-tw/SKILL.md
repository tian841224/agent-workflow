...
**「每一則」涵蓋任務執行中途的進度回報與狀態更新，跟最終回覆套用同一套檢查標準。** 派給 subagent 的英文 prompt、Bash／Agent 等工具呼叫的 `description`／`reason` 欄位可視需要用英文，不受此限；但緊接在這類英文操作之後、要給使用者看的下一則訊息，一律重新套用本節開頭的檢查再輸出。
