...
7. 回報 PASS／FAIL／BLOCKED 表格、execution path、回歸證據、探索結果、波及項與可重現步驟，並原樣附回主 agent 交付的 `diff_sha256`（收尾時會重算比對；驗證期間工作樹被改動會讓它對不上而要求重驗，這正是「驗證基準在過程中被換掉」該被擋下的地方）。不得改 code、設定、依賴或 task status。
