...
3. coordinator／worker task 執行 `~/.agent-workflow/runtime/scripts/close-task.py`（cwd 為自己的 worktree root）結案；它會重跑完整 legacy gate，全數通過才寫 `status: done`。未完成用 `paused` 或 `blocked` 並記錄下一步。被打回時在原 worktree fix-forward，保留失敗證據，不建立新 task。
